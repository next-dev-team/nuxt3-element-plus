...
const getActiveMain = (item: any = {}) => {
  const main
    = item
    && item?.children?.every((i: any) =>
      i.title?.includes(appStore.activeDevMenu.title),
    )
  ...
  <div class="flex border-b py-1 gap-x-2">
    ...
    >
      ...
      >
        ...
        <UnoIcon
          v-if="item?.children"
          class="i-carbon:arrow-down ml-1"
          :class="{ 'text-primary-600': getActiveMain(item) }"
        />
      </MenuButton>
